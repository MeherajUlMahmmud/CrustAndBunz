...
class RegistrationForm(UserCreationForm): # Inherits from the UserCreationForm class 
    ...
    name = forms.CharField(max_length=60, widget=forms.TextInput(attrs={'placeholder': 'Full Name'})) # Add a name field
...
class UpdateOrderForm(ModelForm): # a form to update an order
    class Meta:
        model = OrderModel # Set the model to the OrderModel
        fields = ['shipping_address', 'is_pending', 'is_confirmed', 'is_cooking', 'is_onTheWay', 'is_delivered'] # Set the fields to the above fields
